...
class GCNSAModule(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, n_layers=3, drop_act=False):
        super().__init__()
...
class GCNFPModule(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, n_layers=2, k=3, drop_act=False):
        super().__init__()
        self.k = k
        self.drop_act = drop_act
        self.conv = nn.ModuleList()
...
class GCN_UNet_V1(nn.Module):
    def __init__(self, in_channels, out_channels, idx_list, edge_index_list, global_token_dim=2048):
        super().__init__()
        self.idx_list = idx_list
        self.edge_index_list = edge_index_list

        self.sa1_module = GCNSAModule(in_channels, 32, 64, 3)
        self.sa2_module = GCNSAModule(64+3, 64, 128, 3)

        self.bn_module = GCNSAModule(128+3, 128, 256)

        self.gp1_module = GlobalSAModule(MLP([128 + 3, 512, 1024, global_token_dim]))

        self.fp2_module = GCNFPModule(global_token_dim+256+64, 1024, 512, 3)
        self.fp1_module = GCNFPModule(512 + in_channels, 256, 128, 3)
